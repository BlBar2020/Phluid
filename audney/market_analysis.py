...
def get_stock_price(ticker_symbol):
    try:
        ...
        # Get the historical market data, here max indicates maximum available data
        hist = ticker.history(period="1d")
        # Get the last row of the dataframe (most recent price)
        if not hist.empty:
...
def extract_stock_symbol_with_nlp(text):
    try:
        prompt = f"Identify the company and its stock symbol mentioned in this query: '{text}'"
...
def extract_company_name(text):
    try:
        # Use the OpenAI API to extract the company name
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a knowledgeable financial assistant named Audney. Extract the company name or ticker symbol from the user query."},
        ...
        logger.debug(
            f"GPT-4 response for company name extraction: {response_text}")

        # Simplify the extraction logic to get the last word
        extracted_name_or_symbol = response_text.split()[-1].strip('.').lower()
        logger.debug(
            f"Extracted company name or ticker symbol: {extracted_name_or_symbol}")
        return extracted_name_or_symbol
    except Exception as e:
...
def search_company_or_ticker(query):
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            possible_matches = [(result['name'], result['ticker']) for result in data.get('results', [])]
